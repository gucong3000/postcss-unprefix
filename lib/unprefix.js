...
const declUnprefixers = [
	...
	brotherFirst(RE_BOX_DIRECTION, RE_FLEX_FLOW),
	brotherFirst(/^-\w+-grid-rows$/i, /^(?:-\w+-)?grid-template-rows$/),
	brotherFirst(/^-\w+-grid-columns$/i, /^(?:-\w+-)?grid-template-columns$/),
	brotherFirst(/^-\w+-grid-row$/i, /^(?:-\w+-)?grid-row-start$/),
	brotherFirst(/^-\w+-grid-column$/i, /^(?:-\w+-)?grid-column-start$/),
	valMap(/^-\w+-grid-\w+-align$/i, 'align-self', ALIGN_CONTENT),
	...
	function breakInside (decl) {
		const prop = decl.prop.match(/^(?:-\w+-)?(\w+)-break-(\w+)/i);
		if (prop) {
			const value = decl.value.toLowerCase();
			return {
				prop: 'break-' + prop[2].toLowerCase(),
				value: value === 'avoid' ? 'avoid-' + prop[1].toLowerCase() : value,
			};
		}
	},
	...
	function display (decl) {
		const display = decl.prop.match(/^(-\w+-)?display$/i);
		if (display) {
			let value;
			if ((value = decl.value.match(/^(?:-\w+-)?(inline-)?(?:flex)?box$/i))) {
				value = (value[1] || '') + 'flex';
	...
	function borderRadius (decl) {
		const prop = decl.prop.match(/^(?:-\w+-)?border-radius-(top|bottom)(left|right)$/i);
		if (prop) {
			return {
...
const unprefixValue = util.valueUnprefixer([
	...
	function autoUnprefix (node, prop) {
		...
		if (data && data.props && data.props.some(function (propName) {
