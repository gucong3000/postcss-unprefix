...
function parseNum (string) {
	let unit;
	const num = parseFloat(string.replace(/(\D+)$/, (s) => {
		unit = s;
		return "";
	}));
	if (!isNaN(num)) {
...
/**
 * 将字符串转为px为单位长度数值
 * @param  {string}    str 含有长度信息的字符串
 * @return {number}     换算成像素的数值
 */
function parseLength (str) {
	return /(?:\d*\.)?\d+%?$/.test(str) ? parseFloat(str) : util.length2px(str);
}
...
function fixOldLinearGradient (args) {
	let angle;
	// 从args[0]中获取起始点信息
	const start = normalizePos(args[0]);
	// 从args[1]中获取终止点信息
	const end = normalizePos(args[1]);
...
function fixLinearGradient (args) {
	const angle = parseAngle(args[0][0]);

	if (angle) {
		// 如果第一参数是角度，修正角度
		const deg = Math.round(angle.deg);
		if (deg === 180) {
...
function fixRadialGradient (args) {
	...
	args = args.filter((subArg) => {
		let hasKeyWord;
		subArg.forEach((word, i) => {
			if (keyWordMap[word]) {
				// 查找需要转换的关键字
	...
	node.nodes.forEach((subNode) => {
		if (subNode.type === "div" && subNode.value === ",") {
			...
			if ((subNode.value === "from" || subNode.value === "to" || subNode.value === "color-stop")) {
				let colorStop = subNode.nodes.filter((colorStopInfo) => {
...
module.exports = function (node) {
	...
	const gradient = /^-\w+-((?:\w+-)?(\w+)-gradient)$/i.exec(node.value);
	if (gradient) {
		node.value = gradient[1].toLocaleLowerCase();
		stringifyGradient(node, fixGradient(parseFunction(node), gradient[2]));
	} else if (/^-\w+-gradient$/i.test(node.value)) {
