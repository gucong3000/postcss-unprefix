 ...
 * @return.num {Float}     解析好的数字
 * @return.unit {Float}    原有的单位
 */
...
/**
 * `-webkit-gradient(linear, `根据参数位置与尺寸
...
/**
 * `-webkit-gradient(linear, `根据参数计算角度
 * @param  {Array} args -webkit-gradient的参数，去除第一个参数type和所有color-stop
...
function fixOldGradient(args, gradient) {
	...
	// 将参数中的`color-stop`,`from`,`to`三种信息剔除出来单独存放
...
/**
 * 将二维数组作为参数，重写type为function的语法树节点
 * @param  {Node} node 语法树节点
 * @return {Node}      重写后的语法树节点，type会变为word
 */
