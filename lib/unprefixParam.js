...
function unprefixMedia (params, force) {
	const rule = {};
...
function unprefixSupportsParams (params) {
...
function unprefixSupports (params, force) {
	return unprefixSupportsParams(params) || (force && params);
}

module.exports = function (params, name, force) {
	if (/^media$/i.test(name)) {
		return unprefixMedia(params.toLowerCase(), force);
	}
	if (/^supports$/i.test(name)) {
		return unprefixSupports(params.toLowerCase(), force);
	}
};
